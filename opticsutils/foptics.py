...
def dlSpotSize(r, zi, wl=550e-6):
    ...
    r : float
        radius of the aperture in units of length (usually mm)
    zi : float
        image distance (or the distance of the observation plane) in the same
        units of length as ``r``. For objects at infinity, ``zi`` is the focal
        length of the lens.
    wl : float, optional
        wavelength of light (default=550e-6 mm)
...
def airyPattern(wl, r, zxp, rho, normalization=1):
    """Fraunhoffer intensity diffraction pattern for a circular aperture

    Parameters
    ----------
    wl : float
        wavelength in physical units (same units that of `r`, `zxp`, `rho`)
    r : float
        radius of the aperture
    zxp : float
        distance to the screen/image plane from the aperture/Exit pupil
    rho : ndarray
        radial coordinate in the screen/image plane (such as constructed using
        `meshgrid`)
    normalization : integer (0 or 1 or 2), optional
        0 = None;
    ...
    pattern = ((np.pi*r**2/(wl*zxp))*2*_jinc(2*np.pi*r*rho/(wl*zxp), normalize=False))**2
    if normalization==1:
...
def depthOfFocus(f_number_eff, wavelength=550e-6, first_zero=False, full=False):
    """half diffractive optical focus depth, ``delta_z``, in the image space.
    ...
    ----------
    f_number_eff: float
        effective F-number of the system, given by ``f/D`` for infinite conjugate
        imaging, and ``zi/D`` for finite conjugate imaging; ``f`` is the focal
        length, ``zi`` is the gaussian image distance, ``D`` is the entrance
        pupil diameter.
    wavelength : float, optional
        wavelength of light (default=550e-6, note that the default's unit is mm)
    first_zero : boolean, optional
        Normally the DOF is the region between the focal point (max intensity)
        on the axis and a point along the axis where the intensity has fallen
        upto 20% of the max [Born and Wolf, Page 491, Handbook of Optical Systems. Aberration Theory, Vol-3, Gross, Page 126]. This is the region returned by default by this function. If `first_zero` is True, then the (half-) region returned is from the max intensity to the first zero along the axis.

    Returns
    -------
    delta_z : float
        one-sided depth of focus (in the image space) in the units of the
        ``wavelength``. The total DOF is twice the returned value assuming symmetric
        intensity distribution about the point of focus.
    """
    if first_zero:
...
def _test_fresnelNumber():
    ...
    fresnelNum = fresnelNumber(10, 550e-6, 500, approx=True)
